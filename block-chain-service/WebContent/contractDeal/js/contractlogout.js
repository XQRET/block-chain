...
function loadData(pageInfoList) {
    ...
    layui.each(pageInfoList, function (index, item) {
        ...
        if (item.contractStatus == 40) {
            ...
            html += '<td><span blockId="' + item.id + '" contractId="' + item.contractId + '" status="' + item.contractStatus + '" onclick="waitLogoutCompany(this)" class="bluebtn">' + "待注销" + '</span> </td>';
        } else {
            html += '<td></td>';
        }
        html += '</tr>';
...
//点击待注销状态
function waitLogoutCompany(td) {
    var blockId = $(td).attr("blockId");
    ...
    layer.confirm('确定要注销吗?', {
        ...
        $.ajax({
            type: "POST",
            data: {"userId": userId, "blockId": blockId, "status": 40},
            dataType: 'json',
            url: strRootPath + "/contract/updateWaitLogoutDetail.do",
            success: function (data) {
                if (data != null) {
                    if (data.resultCode == 0) {
                        closefbE();
